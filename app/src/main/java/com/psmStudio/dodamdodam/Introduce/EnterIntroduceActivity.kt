...
import com.psmStudio.dodamdodam.Retrofit2.ResultIntroduceImg
...
class EnterIntroduceActivity : BaseActivity() {
    ...
    private fun updateEnterIntroduce(){
        ...
        val getImage = FileUtils.getFile(this@EnterIntroduceActivity, imageUri)
        ...
        updateCall.enqueue(object : Callback<ResponseBody?> {
            override fun onResponse(
                ...
                if(response.isSuccessful){
                    intent = Intent(this@EnterIntroduceActivity, MainActivity::class.java)
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP)
                    startActivity(intent)
                    finish()
                    Toast.makeText(this@EnterIntroduceActivity, "수정되었습니다.", Toast.LENGTH_LONG).show()
                }else{
                    Toast.makeText(this@EnterIntroduceActivity, "다시 시도 바랍니다.", Toast.LENGTH_LONG).show()
